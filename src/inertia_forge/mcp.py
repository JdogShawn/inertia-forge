...
_TOOLS = [
    ...
    ("forge_xref", "Dangling cross-references — broken imports / orphaned tests", _path_schema("project/pkg"),
     lambda a: ["xref", _p(a)]),
    ("forge_scan", "Full quality battery in one pass (ship/no-ship verdict)", _path_schema("source tree"),
     lambda a: ["scan", _p(a)]),
    ("forge_consistency", "Task-store consistency / drift check", {"type": "object"},
     lambda a: ["consistency"]),
    ("forge_preflight", "Composite readiness gate (tree+plan+arch+consistency)", _path_schema("source dir"),
     lambda a: ["preflight", "--path", _p(a)]),
    ("forge_coverage", "Gate on a Cobertura coverage.xml", _props(file="coverage.xml path"),
     lambda a: ["coverage", a.get("file", "coverage.xml")]),
    ("forge_query", "Aggregate telemetry view (success-rate/outcomes/qc/flaky/estimation-accuracy)",
     _props(view="one of: success-rate, outcomes, qc, flaky, estimation-accuracy"),
     lambda a: ["query", a.get("view", "success-rate")]),
    ("forge_semantic_search", "Semantic recall over the local index", _props(query="search text"),
     lambda a: ["semantic", "search", a.get("query", "")]),
    ("forge_suggest_split", "Recommend how to split an oversized file", _props(file="oversized source file"),
     lambda a: ["suggest-split", a.get("file", "")]),

...
| `inertia-forge engage [run] [--dry-run] [--max-parallel N] [--agent A] [--model M] [--budget T]` / `engage resume <run-id>` / `engage runs` | the **autonomous loop** — walk the task graph wave by wave; per task skip-if-already-satisfied → dispatch → commit → verify-output → done, with a **circuit breaker** on failure ratio and **human-gated pause/resume** (`.forge/engage/runs/`). `--dry-run` runs the whole loop with **zero LLM calls** |

...
| `inertia-forge report [--path DIR]` | one **markdown health page** — the scan-battery verdict per analyzer + code-quality metrics (doc/type %, dead symbols, complexity, coverage) + task progress; for a CI artifact or an LLM |
...
| `inertia-forge ignite <backlog.md>` | ingest a backlog → plan + tasks + suggested agent |
| `inertia-forge ignite run [--dry-run] [--review] [--pr] [--agent P] [--model M] [--max-parallel N]` / `ignite resume <id>` / `ignite runs` / `ignite plan "<goal>"` | the **autonomous pipeline** — walk the task graph wave by wave (Piston implements each task, gated · targeted tests · per-task review), then **finalize**: security-gate the branch (Bastion, fail-closed, P0 blocks) → review it (Caliper/Sentinel/Lattice) → optional PR. **Circuit breaker** + recovery, **human-gated pause/resume**. `ignite plan` is Vector → a structured plan. `--dry-run` runs the whole loop with **zero LLM calls** |
...
| `inertia-forge qc <suite> / validate / list / init` | declarative QC runner — suites with tags, `preconditions`, `${VAR}` interpolation, and tagged scenarios (verdicts passed/failed/skipped; `--tags`/`--skip-tags`). `validate` schema-checks, `list` discovers, `init` scaffolds. Records pass rate + per-scenario history to telemetry |
...
| `inertia-forge calibrate record/accuracy/stats/estimate/duration/model/effort/budget` | estimation calibration — record estimate-vs-actual (`--type`, `--duration`, `--outcome`, `--complexity`); `stats` is the full per-type record (sample count, std_dev, p80, avg duration, success rate, MAPE, complexity/min, recommended tier·model·effort); plus `estimate` (mean+std+p80), `duration` (avg+p80 min), `model`, `effort`, `budget` |
| `inertia-forge models list / recommend --tokens N --family F [--complexity C]` | **LLM-aware** model recommender (recommends, never calls) — family-agnostic tiers mapped to a 2026 registry (anthropic/openai/codex/google/kimi/grok/deepseek/qwen/glm); override in `.forge/models.yaml` |
| `inertia-forge invoke <prompt> [--agent claude/codex] [--model M] [--driver] [--permission-mode P] [--json]` | the **one opt-in LLM bridge** — shell out to a coding-agent CLI (`claude -p … --output-format json`, or `codex exec`), parse result/cost/tokens/session, accumulate against a token budget, and record the call to telemetry. The deterministic core never calls this; it exists for the rare step that wants a model |
| `inertia-forge providers list / show <name>` | the **LLM provider registry** — adapters that make every agent call vendor-agnostic (claude · codex · gemini · cursor-agent · ollama · llm), each a command template + parse rule; add or override any in `.forge/providers.yaml`. The whole forge runs on whatever LLM you configure |
| `inertia-forge dispatch <agent> <context> [--cli claude] [--budget T] [--json]` | invoke a **named agent** from `.claude/agents/<name>.md` through the invoke bridge — applies the agent's model, tools, permission mode, and system prompt, then runs it via `AgentSession` (registry CRUD lives in `subagent`) |
| `inertia-forge handoff [--write]` / `handoff pack <task-id> [--to codex]` / `handoff unpack <pkg>` | the continuity **brief** (ready/blocked/waves/commits/drift), plus a **portable package**: `pack` bundles a task + its files + instructions + token estimate into a `.tgz` for another agent (codex/cursor/generic); `unpack` extracts it |
| `inertia-forge review-agent [diff\|branch\|commit] [--agents caliper sentinel] [--json]` | **agent-backed review** — the forge's review agents over a diff (Caliper quality · Sentinel security · Lattice cross-module on large diffs), combined findings classified to a verdict by P0/P1/P2 severity. Diff fenced as untrusted data; reviewers read-only. Complements the deterministic `review` (smells) and `vet` (insecure code) |

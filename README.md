...
| `inertia-forge calibrate record/accuracy/estimate` | estimation calibration — log estimate-vs-actual pairs (with `--type`); `accuracy` reports MAPE + bias; `estimate <type>` gives a forward baseline (mean + p80) from that type's actuals |

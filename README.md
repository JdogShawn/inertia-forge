...
| `inertia-forge engage [run] [--dry-run] [--max-parallel N] [--review] [--agent A] [--model M] [--budget T]` / `engage resume <run-id>` / `engage runs` | the **autonomous loop** — walk the task graph wave by wave; per task skip-if-already-satisfied → dispatch (with **targeted tests** for changed files) → commit → verify-output → optional **review-and-fix loop** (`--review`) → done, with a **circuit breaker** (+ recovery guidance) on failure ratio and **human-gated pause/resume** (`.forge/engage/runs/`). `--dry-run` runs the whole loop with **zero LLM calls** |

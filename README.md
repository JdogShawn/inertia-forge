...
| `inertia-forge calibrate record/accuracy/estimate/duration/model/effort/budget` | estimation calibration — record estimate-vs-actual (`--type`, `--duration`); per-type `estimate` (mean+std+p80), `duration` (avg+p80 min), `model` (tier recommendation), `effort`, `budget` |
